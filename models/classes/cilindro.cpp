...
cilindro::cilindro(ponto centroBase, ponto centroTopo, float radius, tupla Kd, tupla Ke, tupla Ka, float M, bool temBase, bool temAnel, bool temTopo) : centroBase(centroBase), centroTopo(centroTopo), radius(radius), objeto(Kd, Ke, Ka, M), temBase(temBase), temAnel(temAnel), temTopo(temTopo){colisaoBase = false; colisaoTopo = false;};
